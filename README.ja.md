...
間隔をどう変えても、ダッシュボード側が追随します。実際に収集が何分おきに走っているかを測り、その 1.5 倍を超えたときだけ「取り込みが古い」と言います。固定のしきい値にすると、間隔を変えた瞬間にどちらかが必ず誤ります。また、集計から 10 分以上経っている場合は「直近 1 時間」「直近 24 時間」に集計時刻を併記します。これがないと、単に集計が古いだけの 0 を「使っていない」と読んでしまいます。

**収集間隔とグラフの粒度は別物です。** グラフは常に 1 時間刻みで、これはイベントを実際のタイムスタンプごとに取り込んでいるからです。3 時間おきの収集にしてもグラフが粗くなることはなく、変わるのは「直近の何時間ぶんがまだ未反映か」だけです。間隔は「どれだけ新しい数字が要るか」で決めてください。グラフの細かさとは関係ありません。

この実測を正しく保つために 2 つ仕掛けがあります。どちらも既存ユーザーには注意が必要です。

- 定期タスクは `--scheduled` を付けて実行し、その印が付いた実行だけを間隔の計測に使います。手作業の実行は調べもの中に固まって走るので、混ぜると「普段の間隔」ではなく「いま何回叩いたか」を測ってしまいます。このフラグは鵜呑みにせず裏を取っていて、スケジューラからの実行（Windows では `-InstallTask` が登録する `pythonw.exe`）のときだけ有効です。手で付けても警告が出るだけで記録は変わりません。**この版より前にタスクを登録した場合は、`-InstallTask` を一度実行し直してください。** それまでヘッダーは「自動収集を確認できません」と表示します。登録し直した直後は「測定中」になり、スケジュール実行が必要な回数だけ溜まってから間隔を名乗ります。失敗した実行も間隔の計測には数えます。測りたいのは「タスクが何分おきに起動するか」であって「何分おきに成功したか」ではないので、成功だけを見ると一回おきに失敗している毎時タスクを「2 時間おきの設定」と学習してしまいます。
- 「古い」の判定は、ページを書いた時刻ではなく**最後に取り込みに成功した時刻**で行います。ライブ DB がロックされていたり消えていたりしても集計自体は走るので、生成時刻で測ると何時間も前のデータを「たった今」と表示してしまいます。なお、スケジュール実行から間隔を測れていない間は、しきい値を既定の 90 分より短くはしません。手作業の実行が混ざった実測は実際の間隔より短く出るので、それを信じると正常な 3 時間おきの運用まで「古い」と言ってしまいます。
...
```powershell
.\run-dashboard.ps1 -TestAlert    # テスト通知を送る。出せない場合は理由を言う
.\run-dashboard.ps1 -CheckAlert   # 基準と現在値を表示。通知も状態更新もしない
```

### 通知の見た目

![デスクトップ通知](docs/demo/notification.png)

通知は自分の名前ではなく **PowerShell** の名前で届きます。Windows のトーストを出すには登録済みの AppUserModelID が要り、登録するということは何かをインストールするということです。このツールは何もインストールしない方針なので、PowerShell のものを借りています。覚えておく価値はあります。設定画面で通知を切りたいとき、あるいは**なぜ出ないのかを調べるとき**に探す名前がこれになるからです。

通知は PowerShell 経由の Windows トースト API を使うので、追加パッケージもインストールも不要です。

**Windows が拒否したトーストは失敗として扱います。** トースト API は「実際に表示されたか」を教えてくれません。通知が設定でオフになっていても `Show()` は成功し、画面には何も出ません。これを額面どおり受け取るのが一番まずい結果になります。通知は一度も見られていないのに「送った」と記録され、次に閾値を超えるまで黙ってしまうからです。そこで送信前に `ToastNotifier.Setting` を確認します。Windows 自身に聞く方法で、アプリ単位のオフ、ユーザー単位のオフ、グループポリシーによる禁止を区別できます。どれかに当たれば送信失敗として扱い、メッセージは標準出力に落ち、ダッシュボードにバナーが出て、次回の実行で再試行されます。`-TestAlert` はどれが邪魔しているかを名指しします。

```
[warn] 通知を表示できません: Windows の通知が全体でオフです（設定 > システム > 通知）
```

**送信に成功しても、見えたことの保証にはなりません。** 集中モード（応答不可）、全画面のアプリ、ロック画面、「通知バナーを表示する」だけをオフ — これらはいずれも `Setting` が `Enabled` のままで、`Setting` の守備範囲外なので確認していません。多くの場合は通知センターに残ります。`-TestAlert` が「送信しました」と言うのに見た覚えがないときは、そこを見てください。

デスクトップ通知は Windows のみです。それ以外の環境でも閾値の評価は行われ、ダッシュボードには出ます。トーストが省かれるだけです。

**黙って「届いた」ことにはしません。** 送信そのものに失敗した場合は状態を進めないので、次回の収集で再試行されます。加えて、送れなかったものはダッシュボードにバナーとして出ます。定期タスクは `--quiet` で動くので標準出力を誰も見ていませんが、それでも見落とさないようにするためです。

これとは別に、送信はできたものの「このアプリの通知がブロックされていないか」を Windows が答えてくれないケースが 2 つあります。WinRT のトーストを組み立てられない環境（古い形式のバルーン通知に落ちます。こちらには `Setting` に相当するものがありません）と、この識別子での初回送信（`Setting` がまだ初期化されていません）です。

この 2 つは**重複抑制の上では「送信済み」として扱います** — そうしないと、答えが永久に返らない環境では同じ通知が収集のたびに鳴り続けてしまうためです。ただし「確認できた」とは扱いません。「通知を出せたか確認できていません」という専用のバナーをダッシュボードに出し、`--quiet` でも標準出力に出します。

`"alerts_enabled": false` で全体を止められます。`"baseline_enabled": false` にすると従来どおり固定値だけになります。
