...
- Toasts must be sent from `powershell.exe`, not `pwsh.exe`. The WinRT type accelerator syntax (`[Windows.UI.Notifications..., ContentType=WindowsRuntime]`) is a Windows PowerShell 5.1 feature; PowerShell 7 cannot resolve the type. With `pwsh.exe` first in the probe order, every notification threw, was caught, and silently fell back to a `NotifyIcon` balloon — for weeks, unnoticed, because a balloon looks close enough. The tell is the name on the notification: WinRT toasts say "Windows PowerShell" (the borrowed AUMID), balloons say whatever host sent them.
- The toast API does not report whether anything was displayed. With notifications off for the app, `Show()` succeeds and nothing appears, so `notify()` returned true and the alert marked itself delivered without ever being seen — the one failure mode this tool cannot afford. Check `ToastNotifier.Setting` before showing; it distinguishes `DisabledForApplication`, `DisabledForUser`, and `DisabledByGroupPolicy`. It can also come back `$null`, which was observed on an AUMID that had never been used yet. Whatever the cause, `$null` is not a block, so send anyway — otherwise a fresh machine never gets its first notification — but treat that send as unconfirmed, because Windows did not actually answer the question. Test `$null` before comparing against the enum: `Enabled` is `0`, and `[int]$null` is also `0`.
- Do not try to read or write notification settings through the registry. `Enabled` and `ShowBanner` under `HKCU\...\Notifications\Settings\{AUMID}` look authoritative and are widely cited, but on the Windows 11 machine this was measured on, setting them to `0` changed nothing at all — toasts kept appearing through the WinRT path. `Setting` appears to be backed by `HandlerSettings` in `%LOCALAPPDATA%\Microsoft\Windows\Notifications\wpndatabase.db` under the key `s:toast`: editing that value moved `Setting` between `DisabledForApplication` and `Enabled`. That database is an undocumented implementation detail. Never read it at runtime and never treat editing it as a supported step; it is recorded here only because it was the sole way out of the mess described next.
- `PushNotifications\ToastEnabled = 0` was not reversible by writing `1` back. Writing the value back, restarting `WpnUserService`, and toggling the app in Settings all failed to restore notifications, and the Settings toggle kept displaying "on" while toasts stayed blocked. Recovery on this machine required stopping the service and restoring `s:toast` in `wpndatabase.db`. Whether this is a general Windows behaviour or something specific to this environment is unknown — which is exactly why you must never touch these keys to exercise detection logic. It breaks the user's notifications for every app that borrows the same AUMID, and there may be no way back.
- `Setting` only covers explicit blocks. Focus assist, full-screen apps, the lock screen, and "show banners" being off all leave it reporting `Enabled` while nothing reaches the screen. The README says a successful send does not guarantee it was seen, and points at the notification centre.
- "Sent" and "checked" are different states and the alert bookkeeping has to keep them apart. `send_toast()` returns `None` only when `Setting` answered before the send; the balloon fallback (`"fallback"`) and an uninitialised `Setting` (`"unknown"`) both send without any answer. Counting those as ordinary successes advances the dedup marker and clears the dashboard banner, which is the original silent-failure bug wearing a different hat. `fire()` still advances state on them — otherwise the same alert repeats forever on a machine that will never answer — but records `alert_delivery_failed:{kind}` as `"{iso}|unconfirmed"` so the dashboard keeps saying it could not be verified, and prints to stdout even under `--quiet`. Note what the checked state does *not* mean: `Setting == Enabled` says only that nothing has explicitly blocked the app, never that anything reached the screen. Do not let the wording drift from "we could check for a block" to "it was delivered".
- Alerts must fire *before* the payload is written, not after. `alert_delivery_failed` is read while building `meta`, so running `fire()` afterwards publishes the previous run's state: a notification that could not be sent this run stays invisible until the next collection, three hours later. Since unconfirmed sends are deliberately not retried, that banner is the only backstop they have, and a one-cycle delay defeats it. `aic_collect.py` runs `fire()` and re-reads `delivery_failures()` into `payload["meta"]` before `write_atomic`, still wrapped so a notification failure cannot cost you the aggregation.
- Notifications are issued under PowerShell's AppUserModelID because registering our own would mean installing something. That is the name to look for in Settings, and the README has to say so or nobody finds the switch that is muting them.
- A toast cannot be screenshotted with a single capture; the timing never lines up. Fire it from a background job and capture in a loop. Crop to the toast rectangle itself rather than the corner of the screen, or whatever was on screen behind it ships to the repo with it.
